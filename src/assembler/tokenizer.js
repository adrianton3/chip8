...
(function() {
  ...
  Chip8Tokenizer = function(rawString) {
    ...
    chopNumberDec = function(string) {
      var current, ref;
      string.setMarker();
      while (('0' <= (ref = string.getCurrent()) && ref <= '9')) {
        string.advance();
      }
    ...
    chopNumberHex = function(string) {
      var current, rawNumber, ref, ref1;
      string.advance();
      string.advance();
      string.setMarker();
      while (('0' <= (ref = string.getCurrent()) && ref <= '9') || ('A' <= (ref1 = string.getCurrent()) && ref1 <= 'F')) {
        string.advance();
      }
      current = string.getCurrent();
      if (current !== ' ' && current !== '\n') {
    ...
    chopNumberBin = function(string) {
      var current, rawNumber, ref;
      string.advance();
      string.advance();
      string.setMarker();
      while (('0' <= (ref = string.getCurrent()) && ref <= '1')) {
        string.advance();
      }
      current = string.getCurrent();
      if (current !== ' ' && current !== '\n') {
    ...
    chopIdentifier = function(string) {
      var ref, ref1, ref2;
      string.setMarker();
      while (('a' <= (ref = string.getCurrent()) && ref <= 'z') || ('A' <= (ref1 = string.getCurrent()) && ref1 <= 'Z') || ('0' <= (ref2 = string.getCurrent()) && ref2 <= '9')) {
        string.advance();
      }
